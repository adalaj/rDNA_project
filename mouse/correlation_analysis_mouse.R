#Task is to find coorelation between alot of parameters in mouse 
...
rdna_rlfs_table<- fread("BK000964_added_5000nt_IGS_upstream_nontemplate_qmrlfs.out.table.txt", sep="\t", header= TRUE) #71 
...
rdna_rlfs_table_filt<- rdna_rlfs_table[rdna_rlfs_table$actual_RLFS_start >2001 & rdna_rlfs_table$actual_RLFS_start < 47307] #68 
...
rdna_rlfs_table_filt$rDNA_region[rdna_rlfs_table_filt$actual_RLFS_start > 2001 & rdna_rlfs_table_filt$actual_RLFS_start < 5001] <- "Promoter"
sum(rdna_rlfs_table_filt$rDNA_region=="Promoter")
#3

rdna_rlfs_table_filt$rDNA_region[rdna_rlfs_table_filt$actual_RLFS_start > 5000] <- "Promoter and 5'ETS junction"


rdna_rlfs_table_filt$rDNA_region[rdna_rlfs_table_filt$actual_RLFS_start >= 5001] <- "5'ETS"

rdna_rlfs_table_filt$rDNA_region[rdna_rlfs_table_filt$actual_RLFS_start > 9007 ] <- "5'ETS and 18S junction"

rdna_rlfs_table_filt$rDNA_region[rdna_rlfs_table_filt$actual_RLFS_start >= 9008] <- "18S"
rdna_rlfs_table_filt$rDNA_region[rdna_rlfs_table_filt$actual_RLFS_start > 10877 ] <- "18S and ITS1 junction"


rdna_rlfs_table_filt$rDNA_region[rdna_rlfs_table_filt$actual_RLFS_start >= 10878   ] <- "ITS1"
rdna_rlfs_table_filt$rDNA_region[rdna_rlfs_table_filt$actual_RLFS_start > 11877] <- "ITS1 and 5.8S junction"

rdna_rlfs_table_filt$rDNA_region[rdna_rlfs_table_filt$actual_RLFS_start >= 11878 ] <- "5.8S"
rdna_rlfs_table_filt$rDNA_region[rdna_rlfs_table_filt$actual_RLFS_start > 12034 ] <- "5.8S and ITS2 junction"

rdna_rlfs_table_filt$rDNA_region[rdna_rlfs_table_filt$actual_RLFS_start >= 12035] <- "ITS2"
rdna_rlfs_table_filt$rDNA_region[rdna_rlfs_table_filt$actual_RLFS_start > 13122] <- "ITS2 and 28S junction"

rdna_rlfs_table_filt$rDNA_region[rdna_rlfs_table_filt$actual_RLFS_start >= 13123 ] <- "28S"
rdna_rlfs_table_filt$rDNA_region[rdna_rlfs_table_filt$actual_RLFS_start > 17852 ] <- "28S and 3'ETS junction"

rdna_rlfs_table_filt$rDNA_region[rdna_rlfs_table_filt$actual_RLFS_start >= 17853] <- "3'ETS"
rdna_rlfs_table_filt$rDNA_region[rdna_rlfs_table_filt$actual_RLFS_start > 18403 ] <- "3'ETS and IGS junction"

rdna_rlfs_table_filt$rDNA_region[rdna_rlfs_table_filt$actual_RLFS_start > 18404 & rdna_rlfs_table_filt$actual_RLFS_start < 45339] <- "IGS"

fwrite(rdna_rlfs_table_filt, "RLFS_BK000964_added_5000nt_IGS_upstream_master_qmrlfs_table_after_rule.csv", sep = ",")
...
setwd("/Users/jyotiadala/Library/CloudStorage/OneDrive-SUNYUpstateMedicalUniversity/project/bruce_lab/project/rDNA/rloop_and_rdna/mouse/output/files")
riz_count_entire <- fread ("RLFS_BK000964_added_5000nt_IGS_upstream_at_junctn_graphinput.csv", sep = ",", header = TRUE)
...
riz_count_strandwise<- fread("RLFS_KY962518_added_5000nt_IGS_upstream_no_junctn_strandwise_AR_graphinput.csv", sep = ",", header = TRUE)
...
g4s_count<- fread("pG4CS_BK000964_added_5000nt_IGS_upstream_at_junctn_graphinput.csv", sep = ",", header = TRUE)
...
g4s_count_strandwise <- fread("pG4CS_KY962518_added_5000nt_IGS_upstream_no_junctn_strandwise_AR_graphinput.csv", sep = ",", header = TRUE)
...
setwd("/Users/jyotiadala/Library/CloudStorage/OneDrive-SUNYUpstateMedicalUniversity/project/bruce_lab/project/rDNA/coorelation_and_length_distribution_human/mouse/files")
...
for (i in 1:length(count_list)){
  print(paste("mouse_count_distribution_graphinput_", names(count_list)[i], ".csv", sep = ""))
...
for(i in names(count_list)){
  filename<- paste("mouse_count_distribution_graphinput_", i, ".csv", sep = "")
  graph_data<- fread(filename, header = TRUE, sep = ",")
  print(paste(i, "mouse_count_distribution_in_mouse_rdna.tiff", sep = ""))
  ...
  ggsave(paste(i, "_count_distribution_in_mouse_rdna.tiff", sep = ""), 
...
new_rows<- data.table(rDNA_region = c("Promoter","3'ETS","18S","18S","5.8S","5.8S"),
                      strand= c("-","-","+","-", "+", "-"),
                      length_RIZ=c(0, 0, 0, 0, 0, 0),
                      Linker= c(0, 0, 0, 0, 0, 0),
                      length_REZ = c(0, 0, 0, 0, 0, 0),
                      length_RLFS = c(0, 0, 0, 0, 0, 0))


length_distribution <- rbind(length_distribution, new_rows)
fwrite(length_distribution, "RLFS_Length_distribution_entire_mouse_rDNA_master.csv", sep = ",")
...
for ( i in 1: length(list_of_data)){
  ...
  for (j in unique(data_of_interest$rDNA_region)) {
    ...
    avg_rlfs<- round(mean(tmp$length_RLFS),2)  # (round(mean(length_distribution$length_RLFS),2 = 328.91) 
    avg_riz <- round(mean(tmp$length_RIZ),2) #round(mean(length_distribution$length_RIZ),2) = 15.64
    avg_linker <- round(mean(tmp$Linker),2) #round(mean(length_distribution$Linker),2)  = 9.11) 
    avg_rez<- round(mean(tmp$length_REZ),2) #round(mean(length_distribution$length_REZ) = 304.16) 
  ...
  fwrite(length_distribution_graph, paste("mouse_length_distribution_", names(list_of_data)[i], ".csv", sep = ""))
  ...
  fwrite(data_long, paste("mouse_length_distribution_data_long_graphinput_", names(list_of_data)[i], ".csv", sep = ""))
}
...
for(i in names(list_of_data)){
  filename<- paste("mouse_length_distribution_data_long_graphinput_", i, ".csv", sep = "")
  graph_data<- fread(filename, header = TRUE, sep = ",")
  ...
  graph<- ggplot(graph_data, aes(x= rDNA_region, y = round_length, fill= Property)) + 
    geom_bar(stat= "identity", position ="dodge", color = "black") +
    labs(title= paste(i,"_mouse_Length_distribution", sep = ""), 
         x= "Mouse rDNA region", 
  ...
  ggsave(paste(i, "_length_distribution_in_mouse_rdna.tiff", sep = ""), 
...
colnames(g4s_length_distribution)<- c("rDNA_region", "pG4CS_length", "strand")

for ( i in unique(entire_g4s_rdna$rDNA_region)){
  rdna<- entire_g4s_rdna %>% 
    filter (entire_g4s_rdna$rDNA_region == i) %>% 
    select(rDNA_region, pG4CS_length, strand)
...
new_rows<- data.table(rDNA_region = c("Promoter","3'ETS","18S","18S","5.8S","5.8S", "ITS2"),
                      pG4CS_length = c(0, 0, 0,0,0,0,0),
                      strand= c("-","+","+","-", "+", "-", "+"))

g4s_length_distribution <- rbind(g4s_length_distribution, new_rows)
fwrite(g4s_length_distribution, "pG4CS_length_distribution_entire_mouse_rDNA_master.csv", sep = ",")
...
for ( i in 1: length(list_of_data)){
  ...
  for (j in unique(data_of_interest$rDNA_region)) {
    tmp<- data_of_interest %>% filter(data_of_interest$rDNA_region == j)
    region_of_interest<- j
    avg_pG4CS<- round(mean(tmp$pG4CS_length),2)
...
entire_rlfs<- fread("mouse_length_distribution_entire_rDNA_RLFS.csv", sep = ",", header = TRUE)
...
entire_g4s<- fread("mouse_length_distribution_entire_rDNA_pG4CS.csv", sep = ",", header = TRUE)
...
nontemplate_riz<- fread("mouse_length_distribution_nontemplate_rDNA_RLFS.csv", sep = ",", header = TRUE)
...
nontemplate_g4s<- fread("mouse_length_distribution_nontemplate_rDNA_pG4CS.csv", sep = ",", header = TRUE)
...
template_riz<- fread("mouse_length_distribution_template_rDNA_RLFS.csv", sep = ",", header = TRUE)
...
template_g4s<- fread("mouse_length_distribution_template_rDNA_pG4CS.csv", sep = ",", header = TRUE)
...
for (i in 1:length(data_list)){
  ...
  graph_riz_g4<- ggplot(data, aes(x= rDNA_region, y = length, fill= non_canonical_str)) + 
    geom_bar(stat= "identity", position ="dodge", color = "black") +
    labs(title= paste(names(data_list)[i], "_mouse_length_distribution", sep = ""), 
         x= "Mouse rDNA region", 
         y= "Length (Rounded to Nearest Integer)", 
  ...
  ggsave(paste(names(data_list)[i], "_length_distribution_in_mouse_rdna.tiff", sep = ""), 
         plot = graph_riz_g4, width = 18, height = 10, dpi = 150)
}


##G4S vs RIZ correlation
...
rlfs<- fread("RLFS_BK000964_added_5000nt_IGS_upstream_master_qmrlfs_table_after_rule.csv", header = TRUE, sep = ",") #68
...
bin_size <- c(30)#,30, 50, 75, 100, 200, 500, 1000)

#50400 is just to keep X axis as whole number 45306+5000 = [1] 50306, 50306/100 is 503.06

for (i in bin_size){
  bin_size_new<- i +1
  bin_break <- seq(1, 50400, length.out= bin_size_new) # this step is important because it is ensuring consistency across dataset 45306 is the mouse rdna locus
  ...
  rdna_bin<- ggplot() + 
    ...
    scale_x_continuous(breaks = c(seq(0, 50400, by = 5000), 45306), 
                       labels = c(seq(0, 50400, by = 5000), "")) +
    labs(title= "pG4CS vs RIZ frequency distribution in Mouse rDNA", 
         x= "Mouse rDNA region with 100 bins", 
         y= "Frequency", 
         fill= "Non Canonical structures")+
  ...
  ggsave(paste("pG4CS_RIZ_bar_graph_mouse_rdna_in_", i, "bin.tiff", sep = ""), 
  ...
  g4_riz<- ggplot(combined_data, aes(x = pG4CS_counts, y = RIZ_counts)) +
    ...
    labs(title = "Scatter Plot of pG4CS vs RIZ Frequency in Mouse rDNA",
         x = "pG4CS Frequency",
         y = "RIZ Frequency") +
  ...
  ggsave(paste("pG4CS_vs_RIZ_scatter_plot_mouse_rdna_in_", i, "bin.tiff", sep = ""), 
  ...
  riz_g4<- ggplot(combined_data, aes(x = RIZ_counts, y = pG4CS_counts)) +
    geom_point(color = "blue") +
    geom_smooth(method = "lm", se = FALSE, color = "red") +  # Add a trend line
    labs(title = "Scatter Plot of RIZ vs pG4CS in Mouse rDNA",
         x = "RIZ Frequency",
         y = "pG4CS Frequency") +
    scale_x_continuous(breaks= seq(0, graph_max_value, by = 3))+
    scale_y_continuous(breaks= seq(0, graph_max_value, by = 3))+
  ...
  ggsave(paste("RIZ_vs_pG4CS_scatter_plot_mouse_rdna_in_", i, "bin.tiff", sep = ""), 
         plot = riz_g4, width = 18, height = 10, dpi = 150)
  
  print(paste("Mouse_RIZ_vs_pg4CS_scatter_plot_correlation_in_", i, "bins", sep = ""))
...
#Spearman's rank correlation rho in case of 100 bins in seq 1 to 50400
#Spearman's rank correlation rho

#data:  combined_data$RIZ_counts and combined_data$pG4CS_counts
#S = 71674, p-value = 6.068e-10
...
##if you decrease bin size the rho value will increase!
...
##correlation matrix graph is pending. 
#6) Make a correlation matrix between 


setwd("/Users/jyotiadala/Library/CloudStorage/OneDrive-SUNYUpstateMedicalUniversity/project/bruce_lab/project/rDNA/g4s_and_rdna/mouse/pG4CS_at_rdna_output/files")
g4s_count<- fread("pG4CS_KY962518_added_3500nt_IGS_upstream_at_junctn_graphinput.csv", sep = ",", header = TRUE)
g4s_count<- g4s_count[!grepl("junction", g4s_count$rDNA_region),] #nrow=9

setwd("/Users/jyotiadala/Library/CloudStorage/OneDrive-SUNYUpstateMedicalUniversity/project/bruce_lab/project/rDNA/rloop_and_rdna/mouse/one_rDNA_seq/output/files")
riz_count_entire <- fread ("RLFS_KY962518_added_3500nt_IGS_upstream_at_junctn_after_rule_graphinput.csv", sep = ",", header = TRUE)
riz_count_entire<- riz_count_entire[!grepl("junction", riz_count_entire$rDNA_region),] #nrow=9

setwd("/Users/jyotiadala/Library/CloudStorage/OneDrive-SUNYUpstateMedicalUniversity/project/bruce_lab/project/rDNA/coorelation_and_length_distribution_mouse/files")
...
fwrite(cor_matrix, "spearman_correlation_matrix_graphinput_entire_mouse_rdna.csv", sep=",")

png("correlation_plot_entire_mouse_rdna.png", width = 1600, height = 1200, res=150)
corrplot(cor_matrix, type = "lower", method = "square", 
         ...
         title = "Correlation Matrix of Parameters in entire mouse rDNA (not strandwise)", 
         mar=c(0,0,1,0))
# Add title
...
for ( i in names(correlation_filenames)){
  ...
  png(paste("correlation_plot_", i, "mouse_rdna.png",sep = ""), width = 1600, height = 1200, res=150)
  corrplot(cor_matrix, type = "lower", method = "square", 
           addCoef.col = "black",  # Color of correlation coefficient labels
           col = colorRampPalette(c("blue", "white", "red"))(200),  # Custom color palette
           tl.col = "black",       # Text label color
           ...
           number.cex = 1.2,       # Increase correlation coefficient size
           tl.srt = 45,            # Rotate axis labels
           title = paste("Correlation Matrix of Parameters in", i, "mouse rDNA",sep=" "), 
           mar=c(0,0,1,0))
  # Add title
  # Adjust margins to accommodate title

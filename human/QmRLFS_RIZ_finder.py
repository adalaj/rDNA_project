...
class GenerateWindows():
    ...
    def __iter__(self): # must return an iterator!
        return iter(xrange(0,self.length,self.step)) #xrange is python 2 and range is python3
...
parser = argparse.ArgumentParser(prog="python %s"%prog_name, \
                                ...
                                description=textwrap.dedent('''\
                                    Description: 
                                     ...
                                     initiation regions (RIZ) in any DNA sequences'''%(prog_name, version)), \
...
tableheaders_combined = ["#Name", "model", "location", "start_RIZ", "end_RIZ", "length_RIZ", "G_RIZ",
						 "3Gs_RIZ", "4Gs_RIZ", "perc_G_RIZ", "sequence_RIZ","strand"]
						 #"Linker", "start_REZ", "end_REZ", "length_REZ", "G_REZ",
						 #"3Gs_REZ", "4Gs_REZ", "perc_G_REZ", "sequence_REZ", 
########################################################
...
#            "seq": rez_seq}
########################################################
def output_rlfs(riz,  seq, strand, desc, model): #rez,
    ...
    if strand == "+":
        ...
        end = start_i + int(riz['end'])#int(rez['end'])
        blockSize = "%s"%(riz['length']) #,riz['length'])
        blockStarts = "0"
        #blockStarts = "%s,%s"%(0,int(riz['start'])-int(rez['start']))
    else:
        start = start_i + int(riz['start']) # int(rez['start'])
        end = start_i + int(riz['end']) #int(riz['end'])
        blockSize = "%s"%(riz['length'])#,%s%,riz['length']) 
        blockStarts = "0" 
        #blockStarts = "%s,%s"%(0,int(riz['start'])-int(rez['start'])), so i only need one block, thats why i read ,%s% from blockStarts
    ...
    if args.table or table_out:
            outfile_table.write("%s\n"%("\t".join(map(str, [desc, riz['model'], location, riz["start"], riz["end"], riz["length"],
                    ...
                    riz["seq"], strand])))) #linker_length, rez["start"],
                    #rez["end"], rez["length"], rez["G"], rez["G3s"],
                    #rez["G4s"], rez["perc_g"], rez["seq"], 
########################################################
    ...
    if riz_list:  
	    ...
	    ## find REZ for each riz
            if args.outfile != "" and args.verbose:
                ...
                ## RLFS report
            output_rlfs(riz, seq, strand, desc, model) #rez, 
            number_rlfs += 1

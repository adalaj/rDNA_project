...
#
# Purpose:
#   This script calculates sequence identity between human and chicken rDNA
...
#
# Major Steps:
#   1. Load human (*KY962518*) and chicken (*KT445934*) rDNA FASTA sequences.
...
#   5. Save all pairwise results and an identity matrix (human vs. chicken) to CSV files.
#
# Input:Fasta files for human and chicken entire and subregions. 
#
# Output:
#   - human_vs_chicken_rDNA_sequences_global_blast.csv
#       Pairwise alignment statistics between all human and chicken rDNA regions (Supplementary table 7 and fig7 asterisk number).
#
# Notes:
...
#read required files for chicken
...
chicken_coding_rdna<- as.character(chicken_rdna[[1]])
nchar(chicken_coding_rdna)
#11863


chicken<- fread("rdna_chicken_dataset_details_v1.csv", sep = ",", header = TRUE)
#this file has all rDNA region wise sequence and their nucleotide distribution
#7 rows


#calculate pair wise alignment btw chicken and human
...
chicken_tmp<- chicken[c(1:7), c(1,2)]
new_row1 <- data.frame(Name = "chicken_coding_rdna", Sequences = chicken_coding_rdna)
chicken_tmp<- rbind(chicken_tmp, new_row1)
...
fwrite(attempt2, "human_vs_chicken_rDNA_sequences_global_blast.csv")
#8*8 =64
...
colnames(identity_matrix)<- t(chicken_tmp[,1]) #1 is Name
...
for (i in 1:nrow(human_tmp)){
  ...
  for (j in 1:length(chicken_tmp$Name)){
    k <- paste(human_tmp$Name[i], chicken_tmp$Name[j], sep="&") 
...
fwrite(identity_matrix,
       "human_vs_chicken_rDNA_sequences_global_blast_identity_matrix.csv")
